package internal

import (
	"testing"

	"github.com/stretchr/testify/assert"
)

var testPassword = []byte("test-password")
var testSalt = []byte("test-salt")

var expectedKey = Key{
	key: []byte{0x20, 0xb3, 0xc, 0x92, 0x34, 0xc1, 0x97, 0x8f, 0x33, 0xc, 0xe4, 0x9d, 0x47, 0x23, 0x85, 0x41, 0xe2, 0x2f, 0x5a, 0xcc, 0x77, 0x6f, 0xd5, 0x8f, 0x66, 0x4a, 0x55, 0xf4, 0x70, 0xa4, 0xa5, 0x52},
	iv:  []byte{0xd4, 0x69, 0xa8, 0xde, 0x62, 0xb9, 0x54, 0xab, 0xb, 0x6b, 0x75, 0x9c, 0xae, 0xcc, 0x5, 0x61, 0x4d, 0xaf, 0xeb, 0x12, 0xa1, 0xf0, 0x69, 0xcf},
}
var expectedKeyArgon2id = Key{
	key: []byte{0x78, 0xb3, 0x55, 0x5b, 0x67, 0x9d, 0x2, 0x2f, 0x45, 0x83, 0x4c, 0xb3, 0xf, 0x5, 0xa4, 0x2e, 0x97, 0xd3, 0x5e, 0x24, 0x75, 0x38, 0x86, 0xea, 0xe2, 0xfb, 0xb6, 0x68, 0x19, 0xe6, 0xc1, 0x82},
	iv:  []byte{0x6a, 0x9d, 0x39, 0x7, 0x32, 0x9c, 0x61, 0x6d, 0x2d, 0xc7, 0x12, 0x74, 0xbd, 0x97, 0x63, 0x29, 0x97, 0xc, 0x5e, 0x5f, 0x2b, 0x3c, 0x7e, 0xa3},
}
var expectedKeyScrypt = Key{
	key: []byte{0x88, 0xb8, 0x34, 0xf9, 0x94, 0x55, 0x29, 0x54, 0xe2, 0xcc, 0xe3, 0xb6, 0x99, 0x88, 0xa5, 0x6a, 0x23, 0xe6, 0xd6, 0x90, 0x3c, 0x1f, 0xb, 0x10, 0x48, 0x8, 0xad, 0x45, 0x97, 0xd8, 0xa, 0x9b},
	iv:  []byte{0xf6, 0x8f, 0x16, 0x99, 0xcb, 0xc1, 0x25, 0xe, 0xea, 0xe7, 0xdb, 0xc5, 0x61, 0x1a, 0xf2, 0x7f, 0xed, 0x11, 0x27, 0x94, 0xe0, 0xef, 0xcd, 0xab},
}

func Test_DeriveKeyPbkdf2(t *testing.T) {
	key := DeriveKey(testPassword, testSalt)

	assert.Len(t, key.key, keyLength)
	assert.Len(t, key.iv, ivLength)

	assert.EqualValues(t, expectedKey.key, key.key)
	assert.EqualValues(t, expectedKey.iv, key.iv)

	key2 := DeriveKey([]byte("wrong-password"), testSalt)
	assert.NotEqual(t, key.key, key2.key, "different password has to result in a new key")

	key3 := DeriveKey(testPassword, testSalt)
	assert.EqualValues(t, expectedKey.key, key3.key, "always derive the same key with the same inputs")
	assert.EqualValues(t, expectedKey.iv, key3.iv, "always derive the same key with the same inputs")
}

func Test_DeriveKeyArgon2(t *testing.T) {
	key := DeriveKeyArgon2id(testPassword, testSalt)

	assert.Len(t, key.key, keyLength)
	assert.Len(t, key.iv, ivLength)

	assert.EqualValues(t, expectedKeyArgon2id.key, key.key)
	assert.EqualValues(t, expectedKeyArgon2id.iv, key.iv)

	key2 := DeriveKeyArgon2id([]byte("wrong-password"), testSalt)
	assert.NotEqual(t, key.key, key2.key, "different password has to result in a new key")
}

func Test_DeriveKeyScrypt(t *testing.T) {
	key := DeriveKeyScrypt(testPassword, testSalt)

	assert.Len(t, key.key, keyLength)
	assert.Len(t, key.iv, ivLength)

	assert.EqualValues(t, expectedKeyScrypt.key, key.key)
	assert.EqualValues(t, expectedKeyScrypt.iv, key.iv)

	key2 := DeriveKeyScrypt([]byte("wrong-password"), testSalt)
	assert.NotEqual(t, key.key, key2.key, "different password has to result in a new key")
}
